...
void AnalysisWidget::startEngine()
{
    ...
    if (index != -1) {
        ...
        connect(m_engine, SIGNAL(analysisUpdated(const Analysis&)), SLOT(showAnalysis(const Analysis&)));
        m_engine->activate();
...
void AnalysisWidget::toggleAnalysis()
{
    ...
    AppSettings->setValue(QString("/")+objectName()+"/Engine", ui.engineList->currentText());
}
