...
MainWindow::MainWindow() : QMainWindow(), m_playerDialog(0), m_saveDialog(0), m_helpWindow(0),
    m_showPgnSource(false)
{
    ...
    auto act = new QAction(tr("A"), this);
    act->setToolTip("Flip");
    act->setIcon(style()->standardIcon(QStyle::SP_BrowserReload, 0, this));
    connect(act, &QAction::triggered, this, &MainWindow::slotConfigureFlip);
    toolbar->addAction(act);
    toolbar->addSeparator();
    act = new QAction(tr("A"), this);
    act->setToolTip("Play against the engine");
    act->setIcon(style()->standardIcon(QStyle::SP_MediaPlay, 0, this));
    ...
    toolbar->addSeparator();
    act = new QAction(tr("A"), this);
    act->setToolTip("Engine vs. Engine");
    act->setIcon(style()->standardIcon(QStyle::SP_MediaPlay, 0, this));
    connect(act, QOverload<bool>::of(&QAction::triggered), [=](bool b){
        Q_UNUSED(b);
    ...
    toolbar->addSeparator();
    act = new QAction(tr("A"), this);
    act->setToolTip("first");
    act->setIcon(style()->standardIcon(QStyle::SP_MediaSkipBackward, 0, this));
    connect(act, &QAction::triggered, this, &MainWindow::slotGameMoveFirst);
    toolbar->addAction(act);
    act = new QAction(tr("A"), this);
    act->setToolTip("previous");
    act->setIcon(style()->standardIcon(QStyle::SP_MediaSeekBackward, 0, this));
    connect(act, &QAction::triggered, this, &MainWindow::slotGameMovePrevious);
    toolbar->addAction(act);
    act = new QAction(tr("A"), this);
    act->setToolTip("next");
    act->setIcon(style()->standardIcon(QStyle::SP_MediaSeekForward, 0, this));
    connect(act, &QAction::triggered, this, &MainWindow::slotGameMoveNext);
    toolbar->addAction(act);
    act = new QAction(tr("A"), this);
    act->setToolTip("last");
    act->setIcon(style()->standardIcon(QStyle::SP_MediaSkipForward, 0, this));
    connect(act, &QAction::triggered, this, &MainWindow::slotGameMoveLast);
    toolbar->addAction(act);
    toolbar->addSeparator();
    addToolBar(toolbar);
    ...
    m_boardView->setMinimumSize(250, 250);
    ...
    /* Analysis Dock */
    QDockWidget* analysisDock = new QDockWidget(tr("Analysis 1"), this);
    ...
    m_analysis->setObjectName("AnalysisDock1");
    analysisDock->setWidget(m_analysis);
    addDockWidget(Qt::RightDockWidgetArea, analysisDock);
    connect(m_analysis, SIGNAL(addVariation(Analysis)), SLOT(slotGameAddVariation(Analysis)));
    ...
    connect(analysisDock, SIGNAL(visibilityChanged(bool)), SLOT(slotGameAnalysisStop(bool)));
    m_menuView->addAction(analysisDock->toggleViewAction());
    analysisDock->toggleViewAction()->setShortcut(Qt::CTRL | Qt::SHIFT | Qt::Key_A);
    analysisDock->hide();

    QDockWidget* analysisDock2 = new QDockWidget(tr("Analysis 2"), this);
    analysisDock2->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
    analysisDock2->setObjectName("AnalysisDock2");
    m_analysis2 = new AnalysisWidget;
    m_analysis2->setObjectName("AnalysisDock2");
    analysisDock2->setWidget(m_analysis2);
    addDockWidget(Qt::RightDockWidgetArea, analysisDock2);
    connect(m_analysis2, SIGNAL(addVariation(Analysis)), SLOT(slotGameAddVariation(Analysis)));
    connect(this, SIGNAL(boardChange(const Board&)), m_analysis2, SLOT(setPosition(const Board&)));
    connect(this, SIGNAL(reconfigure()), m_analysis2, SLOT(slotReconfigure()));
    connect(analysisDock2, SIGNAL(visibilityChanged(bool)), SLOT(slotGameAnalysisStop(bool)));
    m_menuView->addAction(analysisDock2->toggleViewAction());
    analysisDock2->toggleViewAction()->setShortcut(Qt::CTRL | Qt::SHIFT | Qt::Key_A);
    analysisDock2->hide();

    QDockWidget* analysisDock3 = new QDockWidget(tr("Analysis 3"), this);
    analysisDock3->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
    analysisDock3->setObjectName("AnalysisDock3");
    m_analysis3 = new AnalysisWidget;
    analysisDock3->setWidget(m_analysis3);
    m_analysis3->setObjectName("AnalysisDock3");
    addDockWidget(Qt::RightDockWidgetArea, analysisDock3);
    connect(m_analysis3, SIGNAL(addVariation(Analysis)), SLOT(slotGameAddVariation(Analysis)));
    connect(this, SIGNAL(boardChange(const Board&)), m_analysis3, SLOT(setPosition(const Board&)));
    connect(this, SIGNAL(reconfigure()), m_analysis3, SLOT(slotReconfigure()));
    //connect(analysisDock3, SIGNAL(visibilityChanged(bool)), SLOT(slotGameAnalysisStop(bool)));
    m_menuView->addAction(analysisDock3->toggleViewAction());
    analysisDock3->toggleViewAction()->setShortcut(Qt::CTRL | Qt::SHIFT | Qt::Key_A);
    analysisDock3->hide();
...
QString MainWindow::exportFileName(int& format)
{
    ...
    if (fd.exec() != QDialog::Accepted)
        return QString();
    if (fd.nameFilters().contains("*.tex"))
        format = Output::Latex;
    else if (fd.nameFilters().contains("*.html"))
        format = Output::Html;
...
void MainWindow::setupActions()
{
    ...
    //help->addSeparator();
    help->addAction(createAction(tr("&About DualChessX"), SLOT(slotHelpAbout()), QString(), QString(), QAction::AboutRole));
